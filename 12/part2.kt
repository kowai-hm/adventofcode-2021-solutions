...
fun atMostOneSingleSmallCaveVisitedOnce(path: MutableList<String>): Boolean {
	val frequencies = path.groupingBy {it}.eachCount()
	var moreThanOnce = false
	for(cave in frequencies.keys) {
		if(!isCaveBig(cave) && frequencies.getOrDefault(cave,-1) > 1) {
			if(moreThanOnce) {
				return false
			} else {
				moreThanOnce = true
...
fun determinePath(paths: MutableList<MutableList<String>>, path: MutableList<String>, map: MutableList<CaveConnection>) {
	if(!atMostOneSingleSmallCaveVisitedOnce(path)) {
		return
	}
	for(caveConnection in map) {
		if(caveConnection.firstCave == path.last()) {
			if(caveConnection.secondCave == "end") {
				...
				copyPath.add("end")
				paths.add(copyPath)
			} else if((!isCaveBig(caveConnection.secondCave) && path.count { it == caveConnection.secondCave } <= 1) 
					|| isCaveBig(caveConnection.secondCave)) {
				val copyPath = path.toMutableList()
